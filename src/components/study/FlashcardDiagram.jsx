            ...
            <ul className="space-y-2.5">
              {card.labels.map((label, i) => (
                <li key={i} className="flex items-start gap-2.5 text-xs">
                  <span className="mt-0.5 shrink-0 rounded-md bg-emerald-50 border border-emerald-200 px-2 py-0.5 text-emerald-700 font-semibold whitespace-nowrap">
                    {label.part}
                  </span>
                  <span className="text-ink-secondary">{label.description}</span>
                </li>
              ))}
            </ul>
          ) : (
