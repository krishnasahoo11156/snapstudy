    ...
    <FlashcardFlip
      ...
      back={
        <div className="w-full text-left">
          <p className="mb-3 text-sm text-accent font-semibold">{card.back}</p>
          {card.steps && card.steps.length > 0 && (
            <ol className="space-y-2">
              {card.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-2.5 text-xs text-ink-secondary">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent/10 text-accent font-bold text-xs">
                    {i + 1}
                  </span>
                  <span>{step}</span>
                </li>
