...
## 1.2 Shared TypeScript Contracts (NON-NEGOTIABLE)

Create `src/types/index.ts`. Both branches MUST import from here. **Never** redefine these shapes.
...
export type RegionType = "equation" | "diagram" | "definition" | "list" | "prose";
export type CardType = "qa" | "derivation_steps" | "labeled_diagram" | "timeline";
...
export interface Region {
  id: string; // "region_0", "region_1", etc.
  box_2d: Box2D;
...
export interface Flashcard {
  id: string; // "card_0", etc.
  source_region_id: string; // MUST link to Region.id
  card_type: CardType;
  front: string;
  back: string;
  steps?: string[];           // only for derivation_steps
  labels?: DiagramLabel[];    // only for labeled_diagram
}
...
export interface PhotoRecord {
  ...
  originalPhotoUrl: string; // Firebase Storage URL
  originalPhotoPath: string; // Storage path for server-side crop
  createdAt: Date;
  regions: Region[];
  cards: Flashcard[];
}
...
export interface RemediationResult {
  explanation: string;
  hints: string[];
  referencesSource: boolean; // true if grounded in crop
}
...
│   ├── routes/
│   │   ├── detect.ts         # POST /api/detect-regions
│   │   ├── generate.ts       # POST /api/generate-cards
│   │   └── remediate.ts      # POST /api/remediate
│   ├── services/
│   │   ├── gemini.ts         # Gemini client, model routing
│   │   └── crop.ts           # Image cropping utility
│   └── utils/
│       └── prompts.ts        # All Gemini prompts
├── src/
│   ├── types/
│   │   └── index.ts          # SHARED CONTRACTS (above)
│   ├── data/
│   │   └── mock-data.ts      # Mock generators for Branch B
│   ├── lib/
│   │   ├── firebase.ts       # Firebase init, Auth, Firestore, Storage
│   │   └── api-client.ts     # Typed fetch wrappers (stubs → real)
│   ├── components/
│   │   ├── layout/
│   │   │   ├── MobileLayout.tsx
│   │   │   ├── DesktopLayout.tsx
...
│   │   ├── useAuth.ts
│   │   ├── usePWA.ts         # Branch B
│   │   └── useOffline.ts     # Branch B
│   ├── utils/
│   │   └── coordinates.ts    # Branch A: normalized → pixel conversion
│   ├── App.tsx
│   ├── main.tsx
│   └── index.css
├── vite.config.ts
...
Create `src/lib/api-client.ts`. **Start with mock mode = true.** Branch B builds UI against this. Branch A later sets `MOCK_MODE = false`.
...
async function post<T>(path: string, body: unknown): Promise<ApiResponse<T>> {
  if (MOCK_MODE) {
    ...
    if (path.includes("detect-regions")) return { success: true, data: generateMockRegions() as unknown as T };
    if (path.includes("generate-cards")) return { success: true, data: generateMockCards() as unknown as T };
    if (path.includes("remediate")) return { success: true, data: generateMockRemediation() as unknown as T };
...
export const api = {
  detectRegions: (imageBase64: string) =>
    post<DetectRegionsResponse>("/detect-regions", { image: imageBase64 }),
  generateCards: (regions: unknown[], imageBase64: string) =>
    post<GenerateCardsResponse>("/generate-cards", { regions, image: imageBase64 }),
  remediate: (payload: RemediationPayload) =>
    post<RemediationResult>("/remediate", payload),
};
```
...
```typescript
import type { Region, Flashcard, RemediationResult } from "../types";

export function generateMockRegions(): { regions: Region[] } {
  return {
    regions: [
      {
        id: "region_0",
...
export function generateMockCards(): { cards: Flashcard[] } {
  return {
    cards: [
      {
        id: "card_0",
...
export function generateMockRemediation(): RemediationResult {
  return {
...
## 1.10 App.tsx Skeleton (Baseline)
...
- If a contract in `src/types/index.ts` needs changing, BOTH branches must agree before merge.
...
|--------|-------|-------------|
| Photo Capture | `src/components/capture/CaptureScreen.tsx` | Camera + upload UI. Compress image, upload to Firebase Storage, get download URL. |
| Region Detection API | `server/routes/detect.ts` | `POST /api/detect-regions`. Gemini Call 1. |
| Flashcard Generation API | `server/routes/generate.ts` | `POST /api/generate-cards`. Gemini Call 2. |
| Remediation API | `server/routes/remediate.ts` | `POST /api/remediate`. Gemini Call 3. |
| Gemini Service | `server/services/gemini.ts` | Client init, model routing (`MODEL_BATCH` vs `MODEL_LIVE`), retry logic. |
| Image Cropping | `server/services/crop.ts` | Server-side crop using `sharp`. Normalized → pixel conversion. |
| Coordinate Utilities | `src/utils/coordinates.ts` | Convert `box_2d` (0–1000) to pixel coordinates for any image dimensions. |
| Prompts | `server/utils/prompts.ts` | All Gemini prompts, persona-conditioned. |
| Region Overlay | `src/components/region-overlay/RegionOverlay.tsx` | Draw bounding boxes on original photo. |
...
- `src/hooks/usePWA.ts` — Branch B owns PWA hooks
- `src/hooks/useOffline.ts` — Branch B owns offline UX
...
**File:** `src/components/capture/CaptureScreen.tsx`
...
export function getModel(tier: "batch" | "live") {
...
**Prompt (`server/utils/prompts.ts`):**
...
**File:** `src/components/region-overlay/RegionOverlay.tsx`
...
```typescript
export function flashcardGenerationPrompt(regions: unknown[]) {
...
**File:** `server/services/crop.ts`

```typescript
import sharp from "sharp";
import type { Box2D } from "../../src/types";
...
**File:** `server/routes/remediate.ts`
...
| Module | Files | Description |
|--------|-------|-------------|
| Auth UI | `src/components/auth/AuthScreen.tsx` | Google sign-in, email sign-in UI |
| Mobile Layout | `src/components/layout/MobileLayout.tsx` | Bottom nav, full-screen flows |
| Desktop Layout | `src/components/layout/DesktopLayout.tsx` | Sidebar, split-pane study view |
| Study Dashboard | `src/components/study/StudyDashboard.tsx` | Deck list, progress, "Continue" |
| Flashcard Components | `src/components/study/Flashcard*.tsx` | Type-branched card rendering |
| Quiz Flow | `src/components/quiz/QuizScreen.tsx` | Question, answer, scoring, summary |
| Remediation UI | `src/components/remediation/RemediationScreen.tsx` | Crop + explanation presentation |
| PWA Hooks | `src/hooks/usePWA.ts` | Install prompt, beforeinstallprompt |
| Offline Hook | `src/hooks/useOffline.ts` | Network status, sync indicators |
...
- `src/utils/coordinates.ts` — Branch A owns coordinate math
- `src/lib/api-client.ts` — Baseline owns stubs, but you can flip `MOCK_MODE`
- `src/types/index.ts` — Baseline owns contracts
...
export default function MobileLayout() {
  const [tab, setTab] = useState<"capture" | "study" | "profile">("capture");
...
export default function DesktopLayout() {
  const [view, setView] = useState<"capture" | "study" | "profile">("study");
...
**File:** `src/components/study/StudyDashboard.tsx`
...
**Files:**
- `src/components/study/FlashcardQa.tsx`
- `src/components/study/FlashcardDerivation.tsx`
- `src/components/study/FlashcardDiagram.tsx`
- `src/components/study/FlashcardTimeline.tsx`
- `src/components/study/FlashcardFlip.tsx` (wrapper with 3D flip animation)

**FlashcardFlip.tsx:**
```typescript
import { useState } from "react";

export default function FlashcardFlip({ front, back, children }: { front: React.ReactNode; back: React.ReactNode; children?: React.ReactNode }) {
...
**File:** `src/components/quiz/QuizScreen.tsx`
...
**File:** `src/components/remediation/RemediationScreen.tsx`
...
- [ ] Both: No TypeScript errors, no broken imports
